...
contract("TroveManager - in Recovery Mode", async (accounts) => {
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    assert.equal((await boldToken.balanceOf(dennis)).toString(), lusdAmountD);
    assert.equal((await boldToken.balanceOf(erin)).toString(), lusdAmountE);
    assert.equal((await boldToken.balanceOf(freddy)).toString(), lusdAmountF);
