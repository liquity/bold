...
contract("TroveManager", async (accounts) => {
  ...
  it("redeemCollateral(): cancels the provided Bold with debt from Troves with the lowest interest rates and sends an equivalent amount of Ether", async () => {
    // --- SETUP ---
    ...
    const redemptionAmount = C_netDebt.add(B_netDebt).add(partialRedemptionAmount).add(toBN("5394023488077118211"));
    ...
    const ETHFee = await contracts.troveManager.getEffectiveRedemptionFeeInColl(redemptionAmount, price, 80);
    ...
    assert.equal(bob_debt_After.toString(), BOLD_GAS_COMPENSATION.toString());
    assert.equal(carol_debt_After.toString(), BOLD_GAS_COMPENSATION.toString());
  ...
  it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
    ...
    const redemptionAmount = A_debt.add(B_debt).add(C_debt).add(toBN(11622782534246575341)); // A+B+C+interest
    // .add(BOLD_GAS_COMPENSATION.mul(3))
    ...
    assert.equal(alice_Debt.toString(), BOLD_GAS_COMPENSATION.toString());
    ...
    assert.equal(alice_Status, 5);
    assert.equal(bob_Status, 5);
    assert.equal(carol_Status, 5);
  ...
  it("redeemCollateral(): doesn’t perform partial redemption if resultant debt is > minimum net debt", async () => {
    ...
    // error is due to interest during redemption window
    await th.assertIsApproximatelyEqual(A_debt, toBN(dec(4600, 18)).add(interestA).add(interestB).add(interestC), 6e15);
  });
  ...
  it("redeemCollateral(): hits trove even if resultant debt would be < minimum net debt", async () => {
    ...
    // Check B, C closed and A remains too, despite being partially redeemed and ended up below min debt
    assert.isFalse(await sortedTroves.contains(ATroveId));
    assert.isFalse(await sortedTroves.contains(BTroveId));
    assert.isFalse(await sortedTroves.contains(CTroveId));
    ...
    const A_debt = await troveManager.getTroveDebt(ATroveId);
    // error is due to interest during redemption window
    await th.assertIsApproximatelyEqual(A_debt, toBN(dec(600, 18)).add(interestA).add(interestB).add(interestC), 6e15);
  });
  ...
  it("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
    ...
    const ETHFee = await contracts.troveManager.getEffectiveRedemptionFeeInColl(amount, price, 80);
  ...
  it("redeemCollateral(): doesn’t redeem Troves with ICR < 100%", async () => {
    ...
    await th.redeemCollateralAndGetTxObject(
      carol,
      contracts,
      A_debt.add(toBN("3835889142567224759")),
      10,
      th._100pct,
    );
    ...
    assert.equal(alice_Debt_After.toString(), BOLD_GAS_COMPENSATION.toString());
  ...
  it("redeemCollateral(): caller can redeem their entire BoldToken balance", async () => {
    ...
    const ETHFee = await contracts.troveManager.getEffectiveRedemptionFeeInColl(dec(400, 18), price, 80);
  ...
  it("redeemCollateral(): value of issued ETH == face value of redeemed Bold (assuming 1 Bold has value of $1)", async () => {
    ...
    const ETHFee_1 = await contracts.troveManager.getEffectiveRedemptionFeeInColl(_120_Bold, price, 80);
    ...
    const ETHFee_2 = await contracts.troveManager.getEffectiveRedemptionFeeInColl(_373_Bold, price, 80);
    ...
    const ETHFee_3 = await contracts.troveManager.getEffectiveRedemptionFeeInColl(_950_Bold, price, 80);
    ...
    const activeETH_3 = await activePool.getETHBalance();
    assert.equal(
  ...
  const redeemCollateral3Full1Partial = async (close=false) => {
    ...
    const A_debtToEth = toBN(dec(1900, 18)).add(interestA).div(priceWithoutDecimals);
    const B_debtToEth = toBN(dec(1900, 18)).add(interestB).div(priceWithoutDecimals);
    const C_debtToEth = toBN(dec(1900, 18)).add(interestC).div(priceWithoutDecimals);
    ...
    th.assertIsApproximatelyEqual(
      A_collAfter,
      A_surplus,
      1e13 // due to redemption window interest
    );
    th.assertIsApproximatelyEqual(
      B_collAfter,
      B_surplus,
