...
contract("TroveManager", async (accounts) => {
  ...
  it("batchLiquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
    ...
    assert.isTrue(await troveManager.hasRedistributionGains(C));
    assert.isFalse(await troveManager.hasRedistributionGains(D));
    assert.isFalse(await troveManager.hasRedistributionGains(E));
    ...
    const pendingBoldDebt_C = await troveManager.getPendingBoldDebtReward(C);
  ...
  it.skip("redeemCollateral(): doesn't touch Troves with ICR < 110%", async () => {
    ...
    const { boldAmount: B_boldAmount, totalDebt: B_totalDebt } =
      await openTrove({
        ICR: toBN(dec(133, 16)),
  ...
  it.skip("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - gas) to the redeemer", async () => {
    ...
    */
    ...
    th.assertIsApproximatelyEqual(
      ...
      ETHDrawn.sub(toBN(gasUsed * GAS_PRICE)), // subtract gas used for troveManager.redeemCollateral from expected received ETH
      100000
    );
  });
  ...
  it("hasRedistributionGains(): Returns false it trove is not active", async () => {
    assert.isFalse(await troveManager.hasRedistributionGains(alice));
  });
});
