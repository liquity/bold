...
contract MockLifebuoy is Lifebuoy, Brutalizer {
    ...
    function rescueERC20(address token, address to, uint256 amount) public payable virtual override {
        _brutalizeScratchSpace();
        super.rescueERC20(_brutalized(token), _brutalized(to), amount);
        _checkMemory();
    }

    function rescueERC721(address token, address to, uint256 tokenId) public payable virtual override {
        _brutalizeScratchSpace();
