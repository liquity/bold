...
contract DeployLiquity2Script is StdCheats, MetadataDeployment, Logging {
    ...
    function run() external {
        ...
        vm.writeFile(
            "script/deployment-manifest.json",
        ...
        returns (DeploymentResult memory r)
    {
        ...
        address troveManagerAddress = _computeCreate2Address(
            ...
            address(r.systemParams)
        );

        IERC20Metadata collToken = IERC20Metadata(
        ...
        r.collateralRegistry = new CollateralRegistry(
            IBoldToken(address(r.stableToken)),
            collaterals,
            troveManagers,
            r.systemParams
        );
        ...
        r.contracts = _deployAndConnectCollateralContracts(
            collToken,
            priceFeed,
            addressesRegistry,
            troveManagerAddress,
        ...
        assert(
            address(r.stableTokenV3Impl) ==
                vm.computeCreate2Address(
                    ...
                    keccak256(
                        bytes.concat(
                            type(StableTokenV3).creationCode,
                            abi.encode(true)
                        )
                    )
                )
    ...
    function _deployAndConnectCollateralContracts(
        ...
        contracts.interestRouter = IInterestRouter(
        ...
        addresses.borrowerOperations = _computeCreate2Address(
            ...
            address(contracts.systemParams)
        );
        addresses.troveNFT = _computeCreate2Address(
        ...
        addresses.gasPool = _computeCreate2Address(
        ...
        IStabilityPool(stabilityPool).initialize(
    ...
    function _computeCreate2Address(
        ...
        return
            vm.computeCreate2Address(
                SALT,
                keccak256(getBytecode(creationCode, _addressesRegistry))
            );
    }
    ...
    function _computeCreate2Address(
        ...
        return
            vm.computeCreate2Address(
                SALT,
                keccak256(
                    getBytecode(creationCode, _addressesRegistry, _systemParams)
                )
            );
    }
            ...
            string.concat(
                ...
                string.concat(
                    ...
                    string.concat(
                        ...
                        string.concat(
                            '"systemParams":"',
                            address(c.systemParams).toHexString(),
                            '",'
                        )
                    ),
                    string.concat(
    ...
    function _getDeploymentConstants(
        ISystemParams params
    ) internal view returns (string memory) {
        return
            string.concat(
                "{",
                string.concat(
                    string.concat(
                        ...
                        params.ETH_GAS_COMPENSATION().toString(),
                        '",'
                    ),
                    string.concat(
                        '"INTEREST_RATE_ADJ_COOLDOWN":"',
                        params.INTEREST_RATE_ADJ_COOLDOWN().toString(),
                        '",'
                    ),
                    string.concat(
                        '"MAX_ANNUAL_INTEREST_RATE":"',
                        params.MAX_ANNUAL_INTEREST_RATE().toString(),
                        '",'
                    ),
                    string.concat(
                        '"MIN_ANNUAL_INTEREST_RATE":"',
                        params.MIN_ANNUAL_INTEREST_RATE().toString(),
                        '",'
                    ),
                    string.concat(
                        '"MIN_DEBT":"',
                        params.MIN_DEBT().toString(),
                        '",'
                    ),
                    string.concat(
                        '"SP_YIELD_SPLIT":"',
                        params.SP_YIELD_SPLIT().toString(),
                        '",'
                    ),
                    string.concat(
                        '"UPFRONT_INTEREST_PERIOD":"',
                        params.UPFRONT_INTEREST_PERIOD().toString(),
                        '"'
                    ) // no comma
                ),
                "}"
    ...
    function _getManifestJson(
        ...
        string memory part1 = string.concat(
            ...
            string.concat(
                '"boldToken":"',
                address(deployed.stableToken).toHexString(),
                '",'
            ),
            string.concat(
                '"hintHelpers":"',
        ...
        string memory part2 = string.concat(
            string.concat(
                '"stableTokenV3Impl":"',
                address(deployed.stableTokenV3Impl).toHexString(),
                '",'
            ),
            string.concat(
                '"stabilityPoolImpl":"',
                address(deployed.stabilityPoolImpl).toHexString(),
                '",'
            ),
            string.concat(
                '"systemParams":"',
                address(deployed.systemParams).toHexString(),
                '",'
            ),
            string.concat(
                '"multiTroveGetter":"',
        ...
        string memory part3 = string.concat(
            string.concat(
                '"fpmm":"',
                address(deployed.fpmm).toHexString(),
                '",'
            ),
