...
seconds_within_epoch=$(call_u256 "$governance" 'secondsWithinEpoch()')
if [ "$seconds_within_epoch" -gt "$cutoff" ]; then
    echo Nothing to do, already in cutoff period.
    exit 0
fi

epoch_start=$(call_u256 "$governance" 'epochStart()')
cast -q rpc evm_setNextBlockTimestamp $((epoch_start + cutoff + 1)) || die Failed to set block timestamp.
cast -q rpc evm_mine || die Failed to mine block.
