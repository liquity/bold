...
contract TroveManager is LiquityBase, ITroveManager, Ownable, CheckContract {
    ...
    **/
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        vars.weightedRecordedTroveDebt = getTroveWeightedRecordedDebt(_borrower);
        ...
        _closeTrove(_borrower, Status.closedByLiquidation, vars.weightedRecordedTroveDebt);
    ...
    */
...

    {
        ...
        if (newDebt == BOLD_GAS_COMPENSATION) {
            ...
            _removeStake(_borrower);
            _closeTrove(_borrower, Status.closedByRedemption, weightedRecordedTroveDebt);
    ...
    */
        ...
        override
    {
        ...
        while (currentBorrower != address(0) && totals.remainingBold > 0 && _maxIterations > 0) {
            ...
            _getAndApplyRedistributionGains(contractsCache.activePool, contractsCache.defaultPool, currentBorrower);
    ...
    function getAndApplyRedistributionGains(address _borrower) external returns (uint256, uint256) {
        ...
        return _getAndApplyRedistributionGains(activePool, defaultPool, _borrower);
    }

    // Add the borrowers's coll and debt rewards earned from redistributions, to their Trove
    function _getAndApplyRedistributionGains(IActivePool _activePool, IDefaultPool _defaultPool, address _borrower) internal returns (uint256, uint256) {
        uint256 pendingETHReward;
        uint256 pendingBoldDebtReward;

        if (hasRedistributionGains(_borrower)) {
            _requireTroveIsActive(_borrower);
    ...
    function hasRedistributionGains(address _borrower) public view override returns (bool) {
        /*
    ...
    function getEntireDebtAndColl(
        ...
        view
        override
        returns (uint entireDebt, uint entireColl, uint pendingBoldDebtReward, uint pendingETHReward, uint pendingBoldInterest)
    {
        uint256 recordedDebt = Troves[_borrower].debt;
        uint256 recordedColl = Troves[_borrower].coll;

        pendingBoldDebtReward = getPendingBoldDebtReward(_borrower);
        pendingBoldInterest = calcTroveAccruedInterest(_borrower);
        pendingETHReward = getPendingETHReward(_borrower);

        entireDebt = recordedDebt + pendingBoldDebtReward + pendingBoldInterest;
    ...
    function _redistributeDebtAndColl(IActivePool _activePool, IDefaultPool _defaultPool, uint _debt, uint _coll) internal {
        ...
        */
        ...
        _activePool.decreaseRecordedDebtSum(_debt);
        _defaultPool.increaseBoldDebt(_debt);
        _activePool.sendETH(address(_defaultPool), _coll);
    }

    function closeTrove(address _borrower, uint256 _weightedRecordedTroveDebt) external override {
        _requireCallerIsBorrowerOperations();
        return _closeTrove(_borrower, Status.closedByOwner, _weightedRecordedTroveDebt);
    }

    function _closeTrove(address _borrower, Status closedStatus, uint256 _weightedRecordedTroveDebt) internal {
        ...
        activePool.changeAggWeightedDebtSum(_weightedRecordedTroveDebt, 0);
    }
    ...
    */
    ...
    function setTrovePropertiesOnOpen(address _borrower, uint256 _coll, uint256 _debt, uint256 _annualInterestRate) external returns (uint256) {
        ...
        _updateTroveDebtAndInterest(_borrower, _debt, _annualInterestRate);
    ...
    function updateTroveDebtAndInterest(address _borrower, uint256 _entireTroveDebt, uint256 _newAnnualInterestRate) external {
        _requireCallerIsBorrowerOperations();
        _updateTroveDebtAndInterest(_borrower, _entireTroveDebt, _newAnnualInterestRate);
    }

    function _updateTroveDebtAndInterest(address _borrower, uint256 _entireTroveDebt, uint256 _newAnnualInterestRate) internal {
        _updateTroveDebt(_borrower, _entireTroveDebt);
    ...
    function _updateTroveDebt(address _borrower, uint256 _entireTroveDebt) internal {
