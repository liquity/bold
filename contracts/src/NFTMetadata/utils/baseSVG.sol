...
library baseSVG {
    ...
    function _dynamicTextEls(uint256 _debt, uint256 _coll, uint256 _annualInterestRate, address _owner)
        internal
        pure
        returns (string memory)
    {
        return string.concat(
            _formattedDynamicEl(LibString.toString(_coll), 256, 360),
            _formattedDynamicEl(LibString.toString(_debt), 256, 392),
            _formattedDynamicEl(LibString.toString(_annualInterestRate), 256, 420),
            _formattedAddressEl(_owner)
        );
    }
