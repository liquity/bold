...
//   output.forEach((troveList: any, index: number) => {
...
//   const owners = troveOwners.filter((_: any, index: number) => index % 2 === 0).map((owner: any) => owner.result as string | undefined)
//   const troves = troveOwners.filter((_: any, index: number) => index % 2 === 1).map((trove: any) => trove.result as unknown as Trove | undefined)
...
// export async function getTrovesAndOwners(): Promise<ReturnCombinedTroveReadCallData[]> {
//   const { collaterals } = getContracts()
//   const client = getPublicClient()
